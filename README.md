...
> ★ **防死鎖＝單向逃生門**：「閂」係旅側獨有；「開」永遠有出路 —— 入下游 Sheet／Apps Script 刪 `ALLOW_LOCAL_LOGIN`（未設定＝open），
> 唔經旅側、離線都做到；旅側下次握手見到唔一致會出 ⚠ **只提示**（唔會自動改、唔會自動再閂）。
> 三層備援：旅側 → **平台超管（唔靠下游登記，鎖唔住）** → Sheet 級手動解鎖（最後一層）；見建構計劃 §4.5.1。
