...
export function render(el, params, query = {}) {
  ...
  if (tab === 'inbox') {
    ...
    ${card({ title: '★ 三層備援：邊個鎖得住、邊個鎖唔住', sub: '旅側死 → 平台超管 → Sheet 級手動解鎖（最後一層）', body: `
    <ul class="doc">
      <li><b>第一層 旅側（旅長／教練員）</b>：控制面本體 —— GAS 停權／Vercel 部署壞／sig 密鑰唔見／人冇接手，就會跟住壞。</li>
      <li><b>第二層 平台超管（你）</b>：驗身<b>唔經下游 SHEET 登記</b> —— 某團未登記／紅燈／閂咗／接駁斷，都<b>鎖你唔住</b>；你入得返呢度重設旅長、補登記、輪換金鑰，再叫旅長開閘。限制：要平台＋網絡。</li>
      <li><b>第三層 Sheet 級手動解鎖（逃生門）</b>：該團 Sheet／Apps Script 專案擁有者，刪 <span class="mono">ALLOW_LOCAL_LOGIN</span>（未設定＝open）—— <b>離線、唔靠任何服務</b>，真正最後一層。</li>
