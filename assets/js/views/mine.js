...
export function render(el) {
  ...
  ${chips([
    ...
    { k: '支部系統登入', v: gateMeta(S.branchGate(s.branchId)).label, tone: gateMeta(S.branchGate(s.branchId)).tone, hint: '由旅側控制（支部系統冇掣）' },
  ...
  el.querySelector('#mi-enter').onclick = () => {
    ...
    modal({
      ...
      <div class="kv mt-12">
        ...
        <dt>支部系統登入</dt><dd>${badge(gateMeta(gateOfLink(b?.link)).label, gateMeta(gateOfLink(b?.link)).tone, true)} <span class="xs faint">由旅側控制；閂咗＝支部系統唔可以自己登入，旅入口照入</span></dd>
